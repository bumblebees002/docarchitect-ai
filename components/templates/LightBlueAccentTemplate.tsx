...
const LightBlueAccentTemplate: React.FC<Props> = ({ data, theme, isMini = false, onUpdate }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Use theme color from props
  const accentColor = `#${theme.hex}`;
  const lightBlueBg = `#${theme.hex}15`;
    ...
    <div className="mb-4 group">
      <div className="flex items-center gap-2">
        <Editable 
          text={title} 
          path={path} 
          className="text-[12pt] font-bold uppercase tracking-wide text-slate-900"
          onBlur={handleBlur} 
        />
        {onAdd && (
      ...
      <div className="h-0.5 w-full mt-1" style={{ backgroundColor: accentColor }}></div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex">
        ...
        <div className="w-72 p-6" style={{ backgroundColor: lightBlueBg }}>
          {/* Square Photo with blue border */}
          <div className="relative group no-print mb-6">
            <div 
              className="w-full aspect-square bg-white overflow-hidden border-4"
          ...
          <div className="hidden print:block mb-6">
            {p.profilePicture && (
              <div className="w-full aspect-square overflow-hidden border-4" style={{ borderColor: accentColor }}>
                <img src={p.profilePicture} alt="Profile" className="w-full h-full object-cover" />
              </div>
            )}
          </div>
          ...
          <section className="mb-6">
            ...
            <div className="space-y-3 text-[9pt] text-slate-700">
          ...
          <section>
            ...
            <Editable 
              ...
              className="text-[9pt] leading-relaxed text-slate-700" 
              onBlur={handleBlur} 
            />
          </section>
        </div>

        {/* Right Column - Name and Summary */}
        <div className="flex-1 p-8">
          {/* Name */}
          <h1 className="text-[32pt] font-bold mb-4" style={{ color: accentColor }}>
            <Editable text={p.fullName || 'Mohamed Adel'} path="personalInfo.fullName" onBlur={handleBlur} />
          </h1>

          {/* Summary next to photo */}
          <Editable 
            element="p" 
            text={p.summary} 
            path="personalInfo.summary" 
            className="text-[10pt] leading-relaxed text-slate-600" 
            onBlur={handleBlur} 
          />
        </div>
      </div>
      ...
      <div className="px-8 pb-8">
        ...
        {/* Skills */}
        <section>
          <SectionHeader title={st.skills || 'SKILLS'} path="sectionTitles.skills" onAdd={addSkill} />
...
export default LightBlueAccentTemplate;
